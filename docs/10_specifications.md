...
江戸時代以降（慶長元年・1596 年）から令和までの元号を `(元号名, 開始日, 終了日)` のテーブルとして `format/wareki.py` に定義する。終了日は次の元号の開始日前日、現行元号は `None`（無期限）とする。これにより、少なくとも 300 年前の日付を和暦表示できる。
...
既定では図の最も右側に毛筆体フォントで縦書き配置する。標題位置は `right`（右側・縦書き）または `top`（上部・横書き）から選択でき、`top` の場合は図の上部に図幅いっぱいの標題領域を設ける。文言・フォント・文字サイズ・位置オフセットをスタイル設定として保持する。設定変更はキャンバスへ即時反映する。
